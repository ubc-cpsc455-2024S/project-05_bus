...
const groceryQueries = {
    ...
    getOneGrocery: async function (id) {
        try {
            const grocery = await Groceries.findOne({ id: id });
            return grocery;
        } catch (error) {
    ...
    updateGrocery: async function (groceryData) {
        try {
            const result = await Groceries.updateOne({ id: groceryData.id }, groceryData);
            return result;
        } catch (error) {
            console.error(`Error updating grocery with id ${groceryData.id}:`, error);
            throw error;
        }
    },
    deleteGrocery: async function (id, groupID) {
        try {
            const result = await Groceries.deleteOne({ id: id });
            return result;
        } catch (error) {
    ...
    deleteManyGroceries: async function ({ categoryId = null, locationId = null } = {}){
        try {
            const query = categoryId !== null ? { categoryId } : { locationId };
            const result = await Groceries.deleteMany(query);
            return result;
        } catch (error) {
            console.error(`Error deleting groceries with categoryId ${categoryId} or locationId ${locationId}:`, error);
            throw error;
        }
    }    
}
